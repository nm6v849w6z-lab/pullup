...
(async () => {
  ...
  check(typeof team.removeHallOfFame === "undefined" && !team.setRetiredJersey(entry0.id, 9).ok && !team.setRetiredJersey(entry0.id, null).ok && team.hallOfFame[0].retiredNumber === 7, "Hall of Fame et maillot retiré définitifs");
  team.hallOfFame = [];
  ...
  await new Promise(r => setTimeout(r, 400));
  const card = doc.querySelector(".hc-hof-card");
  check(card.classList.contains("retired") && card.querySelector(".hc-hof-jersey").textContent.trim() === "23" && !card.querySelector("input, button"), "maillot n°23 retiré : la carte ne montre plus que le maillot et le nom");
  check(!/\bpts\b|matchs?\b|Sortir/.test(card.textContent), "carte Hall of Fame sans stats ni bouton de sortie");
  ({ league: saved } = await store.loadMultiLeague(multiSavePath));

...
(async () => {
  ...
  check(!r.ok, "2 lettres refusées");
  r = actions.setTeamTrigram(league.teams[a], a, league, { trigram: "rs" }, T0);
  check(!r.ok, "trigramme ≠ 3 lettres refusé");
  ...
  r = actions.setTeamTrigram(league.teams[a], a, league, { trigram: "CL" }, T0);
  check(!r.ok, "un sigle de 2 lettres est refusé");
  ...
  await waitFor(() => /déjà utilisé/.test((doc.getElementById("clubIdentityFeedback") || {}).textContent || ""), "erreur d'unicité affichée");
  ...
  check(/BKG/.test(doc.querySelector("#clubIdentityPanel svg text") ? doc.querySelector("#clubIdentityPanel svg text").textContent : ""), "le logo type affiche BKG");
  check(doc.getElementById("clubTrigramInput").disabled && /30 jours|jour/.test(doc.querySelector(".club-trigram-note").textContent), "champ verrouillé et délai affiché après changement");

  [...doc.querySelectorAll(".tab-btn")].find(x => x.dataset.tab === "salle").click();
  check(doc.querySelector(".sl-hero-title").textContent === win.eval("arenaInfo(teamA.arenaLevel).name"), "titre de la Salle = nom du palier par défaut");
  doc.getElementById("salleRenameToggle").click();
  check(!doc.getElementById("salleRenameForm").classList.contains("hidden"), "formulaire Renommer affiché");
  doc.getElementById("salleArenaNameInput").value = "Le Chaudron";
  doc.getElementById("salleRenameForm").dispatchEvent(new win.Event("submit", { bubbles: true, cancelable: true }));
  await waitFor(() => doc.querySelector(".sl-hero-title").textContent === "Le Chaudron", "titre de la Salle renommé");
  check(true, "la Salle s'appelle « Le Chaudron »");

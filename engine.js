...
class Team {
  ...
  slotPlayerIds(pos) {
    ...
    // Remplaçants du poste : le premier est "le" remplaçant, les suivants
    // des réservistes (carte Temps de jeu, répartition proposée par
    // defaultSlotMinutes). Quand des minutes sont réglées sur le poste,
    // c'est celui qui en a le plus qui est le remplaçant (retour utilisateur
    // 2026-09-26 : "si je mets plus de minutes au réserviste, il doit passer
    // automatiquement remplaçant") ; à égalité, ou sans réglage, du mieux
    // noté au moins bien noté.
    const mins = (this.lineup.minutes && this.lineup.minutes[pos]) || null;
    const minutesOf = p => (mins ? Number(mins[p.id]) || 0 : 0);
    this.players
      .filter(p => p.id !== s && (this.lineup.backupPositions[p.id] || []).includes(pos))
      .sort((a, b) => (minutesOf(b) - minutesOf(a)) || (b.overall() - a.overall()))
  ...
  setSlotMinutes(pos, playerId, minutes) {
    ...
    // Plafond : 40 min au total sur le poste (retour utilisateur 2026-09-26 :
    // "il faut bloquer les minutes pour ne pas pouvoir mettre plus que 40").
    this.lineup.minutes[pos][playerId] = Number.isFinite(m) ? clamp(m, 0, this.slotMinutesRoom(pos, playerId)) : 0;
  }

  // Minutes maximales que `playerId` peut recevoir sur `pos` : 40 moins ce
  // qui est déjà donné aux autres joueurs du poste.
  slotMinutesRoom(pos, playerId) {
    const m = (this.lineup.minutes && this.lineup.minutes[pos]) || {};
    let others = 0;
    Object.entries(m).forEach(([id, v]) => { if (String(id) !== String(playerId)) others += Number(v) || 0; });
    return Math.max(0, 40 - others);
  }
...
function calendarScheduledTimeForRound(calendarStartAt, round, weekMs = CALENDAR_WEEK_MS, slotOffsetsMs = CALENDAR_CHAMPIONSHIP_SLOT_OFFSETS_MS) {
  ...
  if (calendarCountsWholeDays(weekMs, slotOffsetsMs)) {
    return parisWallClockPlusDays(calendarStartAt, (week * weekMs + slotOffsetsMs[slot]) / CALENDAR_DAY_MS);
  }
...
function calendarCountsWholeDays(weekMs, slotOffsetsMs) {
  return weekMs % CALENDAR_DAY_MS === 0 && slotOffsetsMs.every(o => o % CALENDAR_DAY_MS === 0);
}
...
class MatchEngine {
  ...
  freeThrows(shooter, n, events, quarter, clock, team) {
    ...
    // Plancher relevé (retour utilisateur, 2026-09-26 : "relève un peu les
    // planchers [...] le but c'est qu'on ait des matchs sympa à regarder
    // quelle que soit la division. ça reste un jeu") : 0.58 + 0.35 × attr
  ...
  playPossession(offTeam, defTeam, quarter, clock, events, scoreDiff) {
    ...
    // c'est qu'on ait des matchs sympa à regarder quelle que soit la
    // division. ça reste un jeu", puis "ne monte pas si haut pour la
    // division 1, plutôt 80/85 points [...] 87 points en moyenne en
    // euroleague. pars sur ça") : pente 0.0018 par point au-dessus de 60 et
    // 0.0016 en dessous (au lieu de 0.0048 partout). Mesuré avec
    // simulate.js AVANT : Division VI (tier ~0.55, attributs ~30) tournait
    // à 59 pts/équipe, FG2 30 %, FG3 22 %, 16,6 % de matchs < 45 pts — le
    // rythme (~100 possessions) était bon, seule l'efficacité s'effondrait
    // linéairement avec les attributs. L'ordre entre joueurs faibles est
    // conservé (un 40 reste meilleur qu'un 25), seul l'écart absolu est
    // réduit. APRÈS (avec les LF relevés dans freeThrows) : Division VI →
    // 74 pts/équipe, FG2 39 %, FG3 30 %, 0,5 % de matchs < 45 ; référence
    // (tier 0.85-1.15) 83 → 82 pts ; Division I 98 → 87 pts (moyenne
    // Euroleague, cible utilisateur). Les attributs restent discriminants
